...
4. `Repo Guidance`: local rules that shape the implementation, such as owning modules, route-action seams, file-size limits, select contracts, changelog requirements, migration order, test strategy, and Windows-native command expectations.
5. `Target Repository Shape` or `Target Product Shape`: concrete files, routes, modules, data contracts, UI surfaces, or operating model.
...
- Follow the established owner module, route-action seam, component split,
  select contract, migration process, or verification rule relevant here.
...
- Search before locking the plan. Cite local source reports, docs, modules, scripts, migrations, or tests by path.
- Design streams to flow in dependency order. Foundations, schema, contracts, or shared helpers should come before UI, agent/tooling, docs, and final verification streams that depend on them.
...
- Prefer durable policies in engineering, operations, product, or security docs. A retired plan is history, not the living contract.
- Write tasks as verifiable outcomes, not vague intentions. Use "Add route-action tests for..." instead of "Imconfirm tests."
...
- Include repo guidance where it matters: sibling component split files, route-action extraction, `TABLE_SELECTS`, service-role inventory, code-health file-size checks, migration/database verification, runtime-test limits, and Windows-native commands.
- Name scope early: what ships now, what uses provider access, what uses explicit operator action, what stays read-only, and how secrets or customer/employee data stay protected.
- Include migration IDs and database verification commands when schema changes are part of the plan.
- Include changelog requirements when production-meaningful code, docs, scripts, migrations, package metadata, CI, security, or operations behavior changes.
